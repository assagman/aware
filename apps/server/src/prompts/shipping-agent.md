...
Workflow, in order:
1. Inspect current branch, default branch, remotes, worktrees, and git status.
...
9. Cleanup remote branch, local branch, and task worktree after merge. Do not delete uncommitted/untracked user data.
10. Switch to default worktree and pull latest default branch.
...
- If auth, host, permissions, CI policy, merge conflict, dirty default worktree, or destructive cleanup blocks ship, stop and report exact blocker plus next command.
- Keep final report concise: commits, rebase base, PR URL, merge result, cleanup result, default worktree sync result.