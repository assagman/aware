...
const sections = [
	{ id: "agents", title: "Agents", subtitle: "Profiles, auth, global instructions" },
	{ id: "providers", title: "Providers", subtitle: "Auth and model providers" },
	{ id: "instructions", title: "Instructions", subtitle: "Global operating rules" },
];
	...
	return (
		<section className="settings-page settings-modal route-state-page">
			<aside className="settings-nav-panel" aria-label="Settings sections">
				...
				<nav>
					{sections.map((item) => (
						<Link key={item.id} to={item.id === "agents" ? "/settings" : `/settings/${item.id}`} className={item.id === active.id ? "selected" : ""}>
			...
			<div className="settings-content-panel">
				<div className="home-modal-head settings-content-head">
					...
					<Link to="/" className="settings-close-button">×</Link>
				</div>
				{active.id === "agents" ? (
					<Suspense fallback={<div className="settings-loading"><BusyIndicator label="Loading settings" /></div>}>
						<AgentsPage />
					</Suspense>
				) : (
					<div className="home-empty settings-placeholder">
						<h3>{active.title}</h3>
						<p>{active.subtitle}. Backing route exists; detailed editor can attach here.</p>
					</div>
				)}
			</div>
		</section>
